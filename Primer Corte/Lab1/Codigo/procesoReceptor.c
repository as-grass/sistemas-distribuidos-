...
int solicitantes_activos = 0;
pthread_mutex_t mutex_solicitantes = PTHREAD_MUTEX_INITIALIZER;
...
int main(int argc, char* argv[]) {
  ...
  // Inicializar sincronización y crear hilos
  ...
  while (fgets(linea, sizeof(linea), pipe)) {
    linea[strcspn(linea, "\n")] = '\0';
    if (verbose) {
      log_event("Solicitud recibida: %s", linea);
    }

    char operacion, nombreLibro[100], pipeCliente[100];
    int isbn;
    if (sscanf(linea, " %c, %[^,], %d,%s", &operacion, nombreLibro, &isbn,
               pipeCliente) == 4) {
      if (operacion == 'I') {
        pthread_mutex_lock(&mutex_solicitantes);
        solicitantes_activos++;
        int activos = solicitantes_activos;
        pthread_mutex_unlock(&mutex_solicitantes);

        if (verbose)
          log_event("Nuevo PS conectado. Activos ahora: %d", activos);
        continue;  // No es una solicitud de acción, solo registro
      }
      ...
      if (operacion == 'Q') {
        ...
        if (verbose)
          log_event("Un PS ha indicado que finalizó. Restantes: %d", activos);
      ...
      // Enviar respuesta
      FILE* respuesta = fopen(pipeCliente, "w");
      if (respuesta) {
        fprintf(respuesta, "Solicitud %c procesada para \"%s\" (ISBN %d)\n",
                operacion, nombreLibro, isbn);
        fclose(respuesta);
      }
    } else {
      if (verbose) log_event("Formato inválido: %s", linea);
    }
  }

  //  Fin del pipe: todos los PS han terminado
  if (verbose) {
    log_event(" Todos los PS finalizaron. Cerrando receptor...");
  }

  programa_terminado = 1;
  sem_post(&hay_datos);

  // Cancelar hilo de comandos (si está esperando por stdin)
...
void* hilo_consumidor(void* arg) {
  ...
  while (!programa_terminado) {
    sem_wait(&hay_datos);  // puede bloquear, pero está bien si hay datos

    if (programa_terminado) break;  // verificar justo después del semáforo
